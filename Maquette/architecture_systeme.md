...
graph TB
    ...
    subgraph FRONTEND["FRONT-END (Pages PHP)"]
        ...
        subgraph SHARED["Composants partages"]
            HEADER["header.php<br/>Meta SEO, CDN libs, favicon"]
            NAVBAR["navbar.php<br/>Navigation, recherche, mega menu"]
        ...
        subgraph PAGES_PUB["Pages publiques"]
            ...
            PARCOURIR["tout_parcourir.php<br/>Catalogue, filtres, pagination"]
            ...
            ENCHERE["enchere.php<br/>Systeme enchere"]
    ...
    subgraph BACKEND["BACK-END (Scripts PHP)"]
        ...
        ENCH_ACT["php/enchere_actions.php<br/>Auto-bidding +1 euro"]
    ...
    subgraph DATABASE["BASE DE DONNEES"]
        direction TB
        MYSQL[("MySQL 8+<br/>omnes_marketplace<br/>14 tables<br/>InnoDB / utf8mb4")]
    end

    subgraph ASSETS["RESSOURCES STATIQUES"]
        direction LR
        CSS["css/style.css<br/>2000+ lignes<br/>Animations, responsive"]
        SCRIPT["js/script.js<br/>550+ lignes<br/>AJAX, validation, UI"]
...
graph LR
    ...
    subgraph SERVEUR["Serveur PHP 8.x"]
        B["Pages PHP<br/>18 pages<br/>+ 3 includes"]
        C["Scripts PHP<br/>8 fichiers actions<br/>Logique metier"]
        B -->|"require / include"| C
    end

    subgraph BDD["MySQL 8+"]
        D[("omnes_marketplace<br/>14 tables<br/>InnoDB")]
    end
...
flowchart TD
    ...
    TYPE -->|Enchere| BID["Acheteur definit prix max"]
    BID --> AUTO["Systeme auto-encherit<br/>prix actuel + 1 euro"]
